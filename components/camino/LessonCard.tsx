...
export function LessonCard({
  ...
  const face = locked
    ? undefined
    : done
      ? { background: "#58CC02", borderColor: "#46A302" }
      : { background: unit.color, borderColor: unit.deep };
  ...
  return (
    <div className="artop-node-pop absolute bottom-[calc(100%+16px)] left-1/2 z-20 w-[280px] max-w-[calc(100vw-48px)] -translate-x-1/2">
      <div
        style={face}
        className={cn(
          "rounded-[20px] border-b-[6px] p-4 shadow-[var(--shadow-pop)]",
          locked ? "border-[#cfcfcf] bg-[#e5e5e5]" : "text-white"
        )}
      >
        <p
          className={cn(
            "font-display text-[19px] font-extrabold leading-snug tracking-tight",
        ...
        <p
          className={cn(
            ...
            locked ? "text-[var(--color-text-3)]" : "text-white/85"
          )}
        >
        ...
        <button
          ...
          disabled={locked}
          className={cn(
            "artop-press mt-3 flex min-h-[52px] w-full items-center justify-center gap-2 rounded-[16px] font-display text-[16px] font-extrabold tracking-wide",
            locked
              ? "cursor-not-allowed bg-black/10 text-[var(--color-text-3)]"
              : "bg-white hover:bg-white/95"
          )}
          style={locked ? undefined : { color: done ? "#46A302" : unit.color }}
        >
          {locked ? (
            "BLOQUEADA"
          ) : done ? (
      ...
      <span
        aria-hidden
        style={locked ? undefined : { background: done ? "#58CC02" : unit.color }}
